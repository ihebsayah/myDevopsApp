pipeline {
    ...
    environment {
        ...
        DOCKER_USERNAME = 'iheb1603' // <--- À CHANGER : Votre nom d'utilisateur Docker Hub
        REPO_NAME = 'monapp' // <--- À CHANGER : Nom de votre application (si différent)
    ...
    stages {
        ...
        stage('Docker Build Images') {
            steps {
                script { // Added script block for 'def' variable
        ...
        stage('Docker Push to Registry') {
            steps {
                ...
                withCredentials([usernamePassword(credentialsId: 'docker-hub-credentials', passwordVariable: 'DOCKER_PASSWORD', usernameVariable: 'DOCKER_USER')]) {
                    script { // <--- FIX: Added script block here
        ...
        stage('Deploy to Kubernetes') {
            steps {
                ...
                // NOTE: Cette commande 'sed' est exécutée DANS le conteneur Jenkins (Linux), elle est donc compatible.
